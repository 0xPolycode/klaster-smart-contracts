...
export function createWalletOwner(): Wallet {
  const privateKey = keccak256(
    Buffer.from(getBytes(toBeHex(++counter)))
  );
  return new ethers.Wallet(privateKey.toString(), ethers.provider);
...
export function decodeRevertReason(
  ...
  if (methodSig === "0x08c379a0") {
    const [err] = ethers.AbiCoder.defaultAbiCoder().decode(["string"], dataParams);
    ...
    const [code] = ethers.AbiCoder.defaultAbiCoder().decode(["uint256"], dataParams);
...
export const SignMessageLibInterface = new ethers.Interface(
  SignMessageLib
);
